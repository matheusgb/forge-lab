...
def test_io_strategies_return_same_results() -> None:
    async def run() -> None:
        inputs = (1, 2, 3)
...
def test_blocking_call_delays_event_loop_heartbeat() -> None:
    async def run() -> None:
        inputs = (1, 2, 3)
        ...
        assert blocking.max_heartbeat_delay_seconds > (
            concurrent.max_heartbeat_delay_seconds + 0.03
        )

    asyncio.run(run())


def test_cpu_strategies_return_same_results() -> None:
    async def run() -> None:
